...
def __main__():
    ...
    #use the following line to read gzipped vcf-files
    reader = vcf.Reader(gzip.open(sys.argv[1], 'rb'))
   
    #for unzipped vcf-files, use the following line
...
#                    genos.append("A")
        ...
        if 'GQ' in sample.keys() and sample['GQ'] >= _L:
            ...
            genos.append("N")
            continue
#try to include reference homozygotes with quality threshold
        elif 'RGQ' in sample.keys() and sample['RGQ'] >= _N:
            if not _n:
                if sample['GT'] == "0/0":
                    ref += 2
                    genos.append("R")
...
def processArgs(num):
    try: 
        opts, args = getopt.getopt(sys.argv[num:],"q:d:D:L:N:i:t:f:aGvn")
    ...
    for opt, arg in opts:
        if opt == "-q":
            ...
            global _L 
            _L = int(arg)
#new filter for 'RGQ' (quality measure for non-variant sites)
        elif opt == "-N":
            global _N 
            _N = int(arg)
        elif opt == "-i":
            ...
            usage()
        sys.stderr.write(" ".join([s+" = %s;" for s in "qdDLNitafGvn"]) % (_q, _d, _D, _L, _N, _i, _t, _a, _f, _G, _v, _n) + "\n")
...
or if the entire site does not pass the quality or indel filters all individuals are excluded")
    ...
    print("N - INT - "+str(_N)+" - The minimum RGQ for an individual's genotype to be considered valid, if the site is non-variant.")
