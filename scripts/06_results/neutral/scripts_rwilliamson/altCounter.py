'''Script by Robert Williamson.

This program takes in a VCF and calculates the number of heterozygous and homozygous sites in windows along the chromosomes of each sample in the VCF. This can be used to calculate heterozygosity and coverage of sites across the genome. 

Usage & Options:
python altCounter.py VCF [OPTIONS]

option - argument type - default - description
w - INT - 100 - Size of the window to analyze sequence in (base pairs).
i - FLOAT - 0 - Maximum 'Dels' value a site can have and be included in analysis.
d - INT - 20 - Minimum depth to include a site in the analysis, this is the individual, not total site, depth.
D - INT - 60 - Maximum depth to include a site in the analysis, this is the individual, not total site, depth.
q - INT - 90 - Minimum quality to include a site in the analysis.
L - INT - 40 - Minimum likelihood of a particular individual's genotype call to include a site in the analysis.
N - INT - 26 - Number of alleles
Q - NONE - True - Indicates that we should not throw out sites with no genotype qualities (for use with VCF v4.1+) '''
...
def __main__():
    ...
    #modified to read gzipped files:
    #reader = vcf.Reader(gzip.open(sys.argv[1], "rb"))
